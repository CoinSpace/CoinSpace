...
export async function validateSend(options) {
  ...
  try {
    ...
    if (/Invalid address/.test(err.message)) {
      ...
      // eslint-disable-next-line max-len
    } else if (/Insufficient funds for token transaction/.test(err.message) && ['ethereum', 'binance-smart-chain', 'c-chain', 'ethereum-classic'].includes(wallet.crypto.platform)) {
      showError({
        title: translate('Uh Oh...'),
      ...
      if (/Additional funds confirmation pending/.test(err.details)) {
        ...
        // eslint-disable-next-line max-len
      } else if (/Attempt to empty wallet/.test(err.details) && ['ethereum', 'binance-smart-chain', 'c-chain', 'ethereum-classic'].includes(wallet.crypto.platform)) {
        // eslint-disable-next-line max-len
